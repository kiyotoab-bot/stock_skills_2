...
- `get_stock_info(symbol: str) -> dict | None` — Return fundamental data dict for *symbol*, or None on failure.
- `get_stock_news(symbol: str) -> list[dict]` — Return recent news articles for *symbol*.

#### class PriceHistoryProvider
Provider for price history data.

- `get_price_history(symbol: str, period: str='1y') -> pd.DataFrame` — Return OHLCV price history DataFrame for *symbol*.

#### class ScreeningProvider
Provider for bulk equity screening.

- `get_macro_indicators() -> list[dict]` — Return current macro indicator snapshots.
...
- `search_stock_deep(symbol: str, company_name: str, *, context: str='') -> dict | None` — Run deep research for a single stock.
- `search_x_sentiment(symbol: str, company_name: str, *, context: str='') -> dict | None` — Fetch X (Twitter) sentiment for *symbol*.
- `search_industry(theme: str, *, context: str='') -> dict | None` — Run industry/theme research.
- `search_market(market: str, *, context: str='') -> dict | None` — Run market overview research.
- `search_business(symbol: str, company_name: str, *, context: str='') -> dict | None` — Run business model research for *symbol*.
