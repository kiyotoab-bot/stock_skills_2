...
def _mocked_e2e_env(monkeypatch, tmp_path):
    ...
    stock_info = json.loads(STOCK_INFO_FIXTURE.read_text(encoding="utf-8"))
    stock_detail = json.loads(STOCK_DETAIL_FIXTURE.read_text(encoding="utf-8"))
    ...
    (data_dir / "portfolio.csv").write_text(
        SAMPLE_PORTFOLIO.read_text(encoding="utf-8"), encoding="utf-8"
    )
    (data_dir / "cash_balance.json").write_text(
        SAMPLE_CASH.read_text(encoding="utf-8"), encoding="utf-8"
    )
