...
# --- 一括同期 (KIK-712 / KIK-735) ---


def sync_all() -> dict:
    """data/ → GraphRAG の一括同期.

    SKILL.md の「syncして」に対応する便利関数。実体は
    ``src/data/graph_sync.py``（tools/ は薄いファサードに留める）。
    ...
    """
    from src.data.graph_sync import sync_all as _sync_all
    return _sync_all(_project_root)
