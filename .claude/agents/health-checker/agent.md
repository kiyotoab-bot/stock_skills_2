...
r = check_margin_deadline([float(x) for x in df["Close"].dropna()],
                          list(df["Close"].dropna().index))
# r: {peak_date, peak_price, current_price, drawdown_pct,
#     deadline, days_to_deadline, phase, label}
```

制度信用の返済期限は最大6ヶ月。天井で信用買いした投資家の含み損が解消されて
いなければ、期限までに投げ売りが出て戻り売りの重石になる。

| phase | 意味 | 出力の扱い |
|:---|:---|:---|
| `pressure` | 期日まで30日超。重石が残る | 「期日まであとN日」を出す。**買いは慎重側に** |
| `flying` | 期日の14〜30日前。需給整理が先行し底打ちしやすい | **仕込みの目安**として出す |
| `cleared` | 期日通過。その天井分の期日売りは一巡 | 出す（重石が外れた事実） |
| `no_overhang` | 高値更新中 or 下落が浅い | 出さない |
| `unavailable` | データ不足 | 出さない |

⚠️ **現値が天井を下回っている場合にだけ重石になる。** 高値更新中は信用買いに
含み損が無いので投げ売りの動機が生じない。この条件が無いと直近6ヶ月に高値がある
ほぼ全銘柄が「重石あり」になり、警告が意味を失う。

⚠️ **信用倍率と併せて読む。** 期日は「いつ」を、信用倍率は「どれだけ」を示す。
倍率が高く期日が近い銘柄が最も重い（実測 2026-08-11: 8031.T 三井物産は
信用倍率38.5倍かつ期日まで38日）。

**判断はしない。** 「買うべき」「待つべき」は Strategist の仕事。
