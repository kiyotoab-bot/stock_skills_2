...
レビュー対象に日本株の予想PER・予想配当利回りが含まれる場合、以下を確認する。

1. `forecast_source` が `"jquants"` か（会社予想を使っているか）
2. `forecast_suspect` が `True` の銘柄が判断根拠に使われていないか
3. 予想PER・予想配当が**一次情報（決算短信）で裏取りされているか**

2026-08-05、検証5銘柄のうち2件（40%）で yfinance の予想値が誤っており、
うち1件（6436.T アマノ・予想配当250円 vs 会社予想180円）は誤った利回り6.44%のまま
購入候補の推奨に載っていた。**予想値の出所を確認しない推奨は FAIL とする。**
...
レビュー時は全チェックリストを照合し、通っていない項目があれば指摘する。
