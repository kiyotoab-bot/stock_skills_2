...
保有銘柄の投資仮説がまだ有効かをチェックする。テクニカル（SMA50/200, RSI, **ゴールデンクロス/デッドクロス検出**）とファンダメンタル（変化スコア）の2軸で3段階アラートを出力。
...
### health の出力項目
- 銘柄 / 損益率 / トレンド（上昇/横ばい/下降） / **クロスイベント**（ゴールデンクロス/デッドクロス） / 変化の質（良好/1指標↓/複数悪化） / アラート
- アラートがある銘柄の詳細（理由、SMA/RSI値、クロス発生日・経過日数、変化スコア、推奨アクション）

### forecast の出力項目
- 銘柄 / 現在価格 / 楽観シナリオ / ベースシナリオ / 悲観シナリオ / 信頼度
- ニュース要約（公式メディア）
- Xセンチメント分析（ポジティブ/ネガティブ要因）
