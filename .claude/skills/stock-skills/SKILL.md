...
**切り上げの実行閾値（KIK-769 / 2026-08-24 ユーザー判断）**

`check_trailing_stop()` に `shares=保有株数` を渡すと、切り上げが実行に値するかを判定する。

```python
t = check_trailing_stop(closes, book, current_stop, shares=100)
t["should_raise"]    # 式の上で切り上げられるか
t["raise_gain_yen"]  # 切り上げで増える確定利益の総額
t["raise_worth_it"]  # 実行に値するか（MIN_RAISE_GAIN_YEN = ¥3,000 以上）
```

⚠️ **`should_raise` と `raise_worth_it` を混同しない。** 前者は「式がそう言っている」、
後者は「操作する価値がある」。混ぜると台帳と注文がずれた理由が追えなくなる。

⚠️ **`shares` を渡さないと `raise_worth_it` は None（判定不能）**。False（見送り）と
取り違えない。日次チェックでは必ず株数を渡すこと。

金額基準にしたのは、判断の対象が「手間とリスクに見合うか」だから。%基準だと
株価水準によって同じ%でも金額が変わる。実際 2026-08-24 に
8031.T **+¥4,300**（実行）と 7259.T **+¥100**（見送り）が同じ日に出た。

⚠️ 閾値を設けると「塵も積もれば」を捨てることになる。承知のうえの選択。
⚠️ **注文を触るたびに入力ミスの機会が生まれる**（2026-08-04 の7銘柄誤発注、
2026-08-10 の指値→成行）。小さい切り上げのために毎回触るのは割に合わない。

⚠️ **ストップを切り上げたら「新規発注」ではなく「訂正」**で行う。
取消→新規だと、その間だけ無保護になる（期限もリセットされる）。
訂正のたびに注文一覧の実物で突合し `note_type="order-check"` を残す（PO9）。
...
⚠️ **ストップ台帳に手仕舞い済みの銘柄が残る問題は KIK-764 で塞いだ。**
`get_stop_levels()` は取引履歴を見て、売り切った銘柄を `closed=True` / `stop=None`
で返す（ノートは消さないので `raw` に旧ストップが残る）。判定は
「最終売却日 > exit-rule ノートの日付 かつ 差引株数 <= 0」。
危険なのは残ること自体ではなく、**買い直したときに古い簿価ベースのストップが
現行値として復活する**こと。2026-08-17 の棚卸しでは台帳14件中**7件**が
手仕舞い済みだった（6件は 2026-08-04 の誤発注分）。

⚠️ **`closed=False` でも保有中とは限らない。** 購入予定銘柄の仮ストップ
（例: 9104.T 商船三井）は売却履歴が無いので残る。これは意図した挙動であり、
**発注日に本算定して置き換える**こと。保有と突き合わせたいなら
`load_portfolio()` と交差させる。

⚠️ **売却を `save_trade()` で記録していないと閉じられない。** 実際 8267.T イオンは
2026-05-28 に売却済み（observation ノートにのみ記録）で、trade 履歴が無いため
自動判定から漏れた。**売買は必ず trade 履歴に残す。**
...
Step 5b: tactical   — 短期売買枠（KIK-751）。**overdue は省略禁止**
  ↓
Step 6: strategist  — 今月の1回をどう使うか。発注する / 見送る / 枠を埋める作業をする
Step 7: reviewer    — auto_review で自動挿入
```

**短期売買枠 tactical（KIK-751）**

中長期の投入計画とは**別勘定**。`ctx["tactical"]` と `ctx["tactical_realized"]` に入る。

| 項目 | 中長期 core | 短期 tactical |
|:---|:---|:---|
| 月次上限 | 1回 | 2回 |
| 冷却期間 | 4週 | なし |
| 同時保有 | 制限なし | 1銘柄 |
| 保有期限 | なし | **8週 + 年末（12-31）** |
| ストップ | 統一式（0.85σ・局面高値×0.92） | **取得-8% 固定** |
| conviction 認定 | 必要 | 対象外 |
| 集中度判定 | 対象 | **対象外**（分母にも入れない） |

⚠️ **`tactical.overdue` は必ず出す。** 保有期限を過ぎた建玉は損益に関係なく手仕舞う
対象であり、黙らせると「短期のつもりが塩漬けて中長期保有になる」を制度で防げなくなる。
枠を作った目的そのものが失われる。

⚠️ 短期枠の実現損益は `tactical_realized` に分離される。`realized` に混ぜて報告しない。
混ぜると「中長期の投入計画がどれだけ効いたか」が測れなくなる。

⚠️ 取引を保存するときは `save_trade(..., sleeve="tactical")` を渡す。**既定は core** なので
渡し忘れた取引が短期枠に紛れることはないが、逆に**渡し忘れると中長期の冷却期間を
消費する**。短期枠の発注時は必ず指定すること。

**ストップ執行は月次上限の枠外（KIK-763・2026-08-17 ユーザー判断）**

ストップ抵触による売却を記録するときは `limit_exempt=True` と理由を渡す。

```python
save_trade(symbol="6701.T", trade_type="sell", shares=100, price=4609.0,
           currency="JPY", date_str="2026-08-20",
           realized_pnl=71714,
           limit_exempt=True, exempt_reason="stop-loss 4609 triggered")
```

月次上限は churn（裁量売買の乱発）を抑えるための枠であり、リスク管理の出口を
塞ぐために置いたものではない。枠を使い切った月にストップを置けない＝無保護で持つ、
という帰結を避けるために枠外にする。

| | 月次上限 | 冷却期間 | 実現損益 | 総資産・配分・集中度 |
|:---|:---|:---|:---|:---|
| `limit_exempt=True` | **外れる** | 元から売却は対象外 | **計上する** | **反映する** |
| `excluded_dates` | 外れる | 同上 | `excluded_pnl` に分離 | 反映する |

⚠️ **`excluded_dates` を流用しない。** あちらは実現損益も `excluded_pnl` に回すため、
ストップ執行の損益が目標 ¥10,000,000 への進捗から消える。損益は実在する。

⚠️ 既定は `False`。付け忘れた取引は通常どおり枠を消費するので、
「黙って枠が増えている」方向には壊れない。
...
## 今月の発注
<!-- 予定銘柄と pre_order の結果。発注が起きるのは月次だけなのでここで通す -->
<!-- PO9（発注後の突合）は約定報告を受けた時点で判定する。未記録なら FAIL のまま残す -->
